...
class EvaluationCheckContent(BoxLayout):
    def __init__(self, main_layout, **kwargs):
        ...
        self.padding = dp(20)
        self.all_issues = [] #格式:[{"小明":["问题1","问题2"]}]
        self.spacing = dp(15)
        self.standard_data = None  # 存储标准数据
        self.user_buttons = {}
        # 添加标题
        ...
        try:
            ...
            if file_path:
                ...
                if "standard_data" not in self.standard_data:
                    ...
                    self.check_btn.disabled = False  # 启用检查按钮
    ...
    def generate_excel(self, instance):
        try:
            ...
            # 写入人名和标记×(具体项目)
            for row, (name, projects) in enumerate(self.issues_for_excel.items(), start=2):
                # 写入人名
            ...
            self.status_label.text = "导出成功！文件已保存为：填报情况.xlsx"
